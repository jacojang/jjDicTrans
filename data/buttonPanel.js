...
function dict_backend_changed(){
	var dict_backend = document.getElementById("dict_backend");
	for(var i=0; i < dict_backend.options.length; i++){
		var opt = dict_backend.options[i];
		if(opt.selected == true){
			addon.port.emit('update_prefs',"dict_backend",opt.value);
		}
	}
}
function trans_backend_changed(){
	var trans_backend = document.getElementById("trans_backend");
	for(var i=0; i < trans_backend.options.length; i++){
		var opt = trans_backend.options[i];
		if(opt.selected == true){
			addon.port.emit('update_prefs',"trans_backend",opt.value);
		}
	}
}
...
addon.port.on("init_load",function(prefs){
	...
	for(var i=0; i < dict_backend.options.length; i++){
		var opt = dict_backend.options[i];
		if(opt.value == prefs["jjdict.dict_backend"]){
			opt.selected = true;
		}else{
			opt.selected = false;
		}
	...
	for(var i=0; i < trans_backend.options.length; i++){
		var opt = trans_backend.options[i];
		if(opt.value == prefs["jjdict.trans_backend"]){
			opt.selected = true;
		}else{
			opt.selected = false;
		}
