	...
	en:{
		dict:{
			naver:{
				url:function(txt){
					var a = new Array();
					a.push("http://endic.naver.com/popManager.nhn?sLn=kr&m=search&query="+encodeURI(txt)+"&searchOption=entry_idiom&isOnlyViewEE=N");
					a.push(false);
					return a;
				},
      			size:{w:410, h:440 }
			...
			daum:{
				url:function(txt){
					var a = new Array();
					a.push("http://small.dic.daum.net/search.do?q="+encodeURI(txt)+"&dic=eng&search_first=Y");
					a.push(false);
					return a;
				},
				size:{w:410, h:440 }
		...
		trans:{
			naver:{
				url:function(txt){
					...
					var a = new Array();
					a.push("http://translate.naver.com/#/en/ko/");
					a.push({oid:id,text:txt,btn:"startTranslateBtn"});
					return a;
				},
      			size:{w:900, h:420 }
			},
			google:{
				url:function(txt){
					var id="source";
					var a = new Array();
					a.push("https://translate.google.co.kr/#en/ko/");
					a.push({oid:id,text:txt});
					return a;
				},
      			size:{w:800, h:420 }
			}
			...
			if(state.checked){
				...
				buttonPanel.port.on("update_prefs",function(type,value){
					if(type=="enable"){
						...
						jjDict.prefs["jjdict.mode"] = value;
					}else if(type=="popup_pos"){
						jjDict.prefs["jjdict.popup_pos"] = value;
					}
				});
				...
				buttonPanel.port.on("open_panel",function(type, keyword){
					if(type == "dict"){
			      ...
			      var t_url = backend.url(keyword);
					var pos = backend.size;
					var url = t_url[0];
					var text = t_url[1];
			  	...
			  	if(text && text.oid && text.oid.length > 0){
					...
					jjDict.panel = require("sdk/panel").Panel({
						...
						contentStyle:"body { background-color:white; border: 1px solid #EEEEEE; }",
...
// 초기 값 셋팅
{
	...
	if(p["jjdict.mode"] == undefined || p["jjdict.mode"] == ""){p["jjdict.mode"] = 0; }
	if(p["jjdict.popup_pos"] == undefined || p["jjdict.popup_pos"] == ""){p["jjdict.popup_pos"] = 0; }
...
require("sdk/page-mod").PageMod({
	...
	onAttach: function(work) {
		...
		work.port.on('jjdict.request', function(text,type,pos){
			// URL Make
			if(type == "dict"){
				var backend = backendSet[p['jjdict.lang.from']].dict[p['jjdict.backend.dict']];
			}else{
			...
			if(!pos || pos == undefined || p['jjdict.popup_pos'] == 0){
				pos = jjDict.button;
			}else{
				tleft = pos.left - (sz.w/2);
				ttop = pos.top - (sz.h + 15);
				if(ttop < 0){ ttop = pos.top + 15; }
				pos.left = tleft;
				pos.top = ttop;
			}
			...
			jjDict.panel = require("sdk/panel").Panel({
				contentURL:jjDict.data.url(url),
				contentStyle:"body { background-color:white; border: 1px solid #EEEEEE; overflow-x:hidden; }",
				contentScriptFile: jjDict.data.url('mainPanel.js'),
				contentScript:script
			});
			jjDict.panel.port.emit("init_platform",jjDict.system.platform);
